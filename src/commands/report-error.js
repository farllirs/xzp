...
import { readBooleanEnv } from '../core/config.js';
...
export async function handleCliError(error, argv = []) {
  ...
  const agentMode = readBooleanEnv('XZP_AGENT_MODE', false);
...
function printReportResult(report, delivery, lastError, outputFormat = 'text') {
  ...
  console.log('Asunto  : ' + sanitizeTerminalText(report.subject));

  console.log('Preview : ' + sanitizeTerminalText(report.preview.summary || report.preview.title || 'sin preview'));
