...
export async function runInstallCommand() {
  ...
  const directExitCode = await runCommandWithProgress(installCommand, installMode.args, projectRoot, {
...
async function runSafeInstall({
  ...
  if (projectContext.type === 'python') {
    ...
    const rescueExitCode = await runCommandWithProgress(rescuePython, installMode.args, projectRoot, {
  ...
  if (projectContext.type === 'node') {
    ...
    const rescueExitCode = await runCommandWithProgress(installCommand, installMode.args, safeProjectDir, {
...
function runCommandWithProgress(command, args, cwd, options = {}) {
  return new Promise((resolve, reject) => {
    const startedAt = Date.now();
    const expectedPackages = options.packages || inferPackagesFromArgs(args);
    const estimateSeconds = Math.max(6, expectedPackages.length * 4);
    let lastLine = '';
    let discoveredPackage = '';
    let hadErrorLine = '';

    console.log('Xzp Install Progress');
    console.log(`Paso      : ${options.title || 'instalacion'}`);
    console.log(`Gestor    : ${options.manager || sanitizeInstallToken(command)}`);
    console.log(`Esperados : ${expectedPackages.length ? expectedPackages.join(', ') : 'sin lista explicita'}`);
    console.log(`Estimado  : ~${estimateSeconds}s`);
    console.log('');
    ...
    const interval = setInterval(() => {
      ...
      const packageLabel = discoveredPackage || 'resolviendo';
      const status = hadErrorLine ? 'con alertas' : 'ejecutando';
      console.log(`[${formatElapsed(elapsed)}] ${status} :: paquete ${packageLabel} :: ultimo ${truncateLine(lastLine || 'sin salida aun')}`);
    }, 1200);
    ...
    const onChunk = (chunk, isError = false) => {
      ...
      for (const line of lines) {
        ...
        console.log(`[log] ${truncateLine(line)}`);
      }
    };
    ...
    child.on('exit', (code, signal) => {
      ...
      const elapsed = Math.max(1, Math.round((Date.now() - startedAt) / 1000));
      console.log('');
      console.log(`Resultado : ${code === 0 ? 'ok' : 'error'}`);
      console.log(`Duracion  : ${elapsed}s`);
      console.log(`Paquete   : ${discoveredPackage || 'sin detectar'}`);
      if (hadErrorLine) {
        console.log(`Detalle   : ${truncateLine(hadErrorLine)}`);
      }
      resolve(code || 0);
    });
  });
