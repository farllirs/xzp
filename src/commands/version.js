...
export async function runVersionCommand(options = {}) {
  ...
  const latest = await readLatestPublishedVersion(localPackage.name || PACKAGE_NAME);
...
function buildRegistryUrl(packageName) {
  return `https://registry.npmjs.org/${encodeURIComponent(packageName)}/latest`;
}

export async function readLatestPublishedVersion(packageName = PACKAGE_NAME) {
  try {
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), 2500);
    const response = await fetch(buildRegistryUrl(packageName), {
...
export function __testBuildRegistryUrl(packageName) {
