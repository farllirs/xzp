...
export function resolveRuntimePreferences(config = {}) {
  ...
  const requestedOutputFormat = process.env.XZP_OUTPUT_FORMAT || '';
  return {
    theme: normalizeUiTheme(process.env.XZP_THEME || config.ui?.theme),
    density: normalizeUiDensity(process.env.XZP_UI_DENSITY || config.ui?.density),
...
function normalizeUiTheme(theme) {
  return ['ocean', 'forest', 'ember', 'mono', 'paper', 'contrast'].includes(theme) ? theme : DEFAULT_CONFIG.ui.theme;
}

function normalizeUiDensity(density) {
  return ['compact', 'comfortable'].includes(density) ? density : DEFAULT_CONFIG.ui.density;
}

function normalizePromptTheme(theme) {
  return ['ocean', 'forest', 'ember', 'mono'].includes(theme) ? theme : DEFAULT_CONFIG.ui.promptTheme;
}

function normalizeMenuVisualMode(mode) {
  return ['cards', 'compact'].includes(mode) ? mode : DEFAULT_CONFIG.menu.visualMode;
}

function normalizeOutputFormat(outputFormat) {
  return ['text', 'json'].includes(outputFormat) ? outputFormat : DEFAULT_CONFIG.runtime.outputFormat;
}
