export function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (token === '--locale' || token === '--set-locale' || token === 'locale') {
      parsed.command = 'locale';
      const next = argv[index + 1];
      if (next && !next.startsWith('-')) {
        parsed.localeValue = next;
        index += 1;
      }
      continue;
    }
    ...
    if (token === '--agent-context') {
      parsed.command = 'agent-mode';
      parsed.agentAction = 'context';
      continue;
    }

    if (token === '--agent-refresh-context') {
      parsed.command = 'agent-mode';
    ...
    if (token === '-p' || token === '--paste' || token === 'paste') {
      ...
      const next = argv[index + 1];
      if (next && !next.startsWith('-') && (next === 'linux' || next === 'termux')) {
    ...
    if (token === '--android-start') {
      const next = argv[index + 1];
      if (next) {
        parsed.androidStartPath = next;
        index += 1;
      }
      continue;
    }

    if (token === '--android-pick') {
      parsed.command = 'android';
      parsed.androidPickOnly = true;
      continue;
    }
...
export function normalizeParsedArgs(parsed = {}) {
  return {
    ...parsed,
    command: String(parsed.command || '').trim(),
    helpTarget: String(parsed.helpTarget || '').trim(),
    copyMode: String(parsed.copyMode || 'path').trim() || 'path',
    copyTarget: String(parsed.copyTarget || '').trim(),
    pasteTarget: String(parsed.pasteTarget || '').trim(),
    pasteAction: String(parsed.pasteAction || '').trim(),
    pattern: String(parsed.pattern || '').trim(),
    searchType: String(parsed.searchType || '').trim(),
    topic: String(parsed.topic || '').trim(),
    target: String(parsed.target || '').trim(),
    scope: String(parsed.scope || '').trim().toLowerCase(),
    depth: normalizeDepth(parsed.depth),
    limit: normalizeLimit(parsed.limit),
    safeShortcutAction: String(parsed.safeShortcutAction || '').trim(),
    safeShortcutEntry: String(parsed.safeShortcutEntry || '').trim(),
    safeShortcutName: String(parsed.safeShortcutName || '').trim(),
    safePreset: normalizeSafePreset(parsed.safePreset),
    contextAction: String(parsed.contextAction || '').trim(),
    favoriteName: String(parsed.favoriteName || '').trim(),
    androidStartPath: String(parsed.androidStartPath || '').trim(),
    androidPickOnly: Boolean(parsed.androidPickOnly),
    localeValue: String(parsed.localeValue || '').trim().toLowerCase(),
    searchExcludes: Array.isArray(parsed.searchExcludes)
